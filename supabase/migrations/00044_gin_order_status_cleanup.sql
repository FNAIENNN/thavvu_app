-- ============================================================================
-- Migration 00044: Order status follows GIN to 'added_to_stock' + E2E cleanup.
--
-- When an approved GIN (created from a stock order) adds goods to stock,
-- the order group must also move to 'added_to_stock' so the HOD Orders tab
-- and the supervisor View Orders tab show the complete lifecycle:
--   placed -> received (in GIN review) -> added_to_stock
-- ============================================================================
...
-- ---------------------------------------------------------------------------
-- Cleanup of automated E2E artifacts (orders, GIN bills, balances,
-- movements). Storage objects under gin-documents are deleted via the
-- Storage API, not SQL (Supabase blocks direct storage deletes).
-- ---------------------------------------------------------------------------

-- E2E movements (reference the balances below)
