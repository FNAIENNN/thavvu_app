-- ============================================================================
-- Migration 00059: Harden the admin_create_supervisor HOD guard.
--
-- The old guard trusted ONLY auth.users.raw_user_meta_data->>'role', which
-- breaks for legitimate HOD profiles whose metadata lacks the key. The
-- authoritative role source in this app is profiles.role (see public.has_role
-- from 00001); accept EITHER the profile role OR the metadata role, and only
-- for hod/admin — supervisor accounts stay blocked.
-- ============================================================================
...
DECLARE
  ...
  v_emp_id text;
BEGIN
  -- 1) Guard: only authenticated HOD/admin accounts may create supervisors.
  IF v_caller IS NULL THEN
    RAISE EXCEPTION 'Authentication required';
  END IF;
  SELECT COALESCE(raw_user_meta_data->>'role', '') INTO v_caller_role
  FROM auth.users WHERE id = v_caller;
  IF NOT public.has_role(ARRAY['hod', 'admin'])
     AND v_caller_role NOT IN ('hod', 'admin') THEN
