-- ============================================================================
-- Migration 00038: Fix gin_add_to_stock for the live legacy schema.
--
-- The legacy stock tables (stock_items, stock_batch_balances,
-- stock_movements) were created by pre-00021 migrations with TEXT ids
-- (stock_movements.id is BIGINT). gin_add_to_stock originally declared
-- uuid variables, which produced "operator does not exist: text = uuid".
-- This replaces the function with text-typed variables matching the live
-- schema and generates an explicit stock_items.id for new GIN items.
-- Also drops the temporary gin_diag function from 00037.
-- ============================================================================

DROP FUNCTION IF EXISTS public.gin_diag();
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM public.gin_bill_items WHERE gin_bill_id = p_bill_id LOOP
    ...
    -- stock_items.id is TEXT in the live schema, so an explicit id is
    -- generated for newly created items.
