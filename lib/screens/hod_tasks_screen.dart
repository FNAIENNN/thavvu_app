...
class _HODTasksScreenState extends State<HODTasksScreen>
  ...
  int get _totalPoints => _hodTasks
      .where((t) => t['completed'] == true)
      .fold(0, (sum, t) => sum + (t['points'] as int));
  int get _completedCount =>
      _hodTasks.where((t) => t['completed'] == true).length;
  int get _pendingCount => _hodTasks.length - _completedCount;
  double get _completionPercentage =>
      _hodTasks.isNotEmpty ? (_completedCount / _hodTasks.length) * 100 : 0;
        ...
        const Expanded(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 4),
              Text('Track your task completion progress',
                  style:
  ...
  Widget _buildTaskBreakdown() {
    ...
    final dailyCompleted = _hodTasks
        .where((t) => t['type'] == 'Daily' && t['completed'] == true)
        .length;
    final weeklyCompleted = _hodTasks
        .where((t) => t['type'] == 'Weekly' && t['completed'] == true)
        .length;
    final monthlyCompleted = _hodTasks
        .where((t) => t['type'] == 'Monthly' && t['completed'] == true)
        .length;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          _buildBreakdownItem(
              'Daily Tasks', dailyCompleted, dailyCount, AppTheme.info),
          const SizedBox(height: 10),
          _buildBreakdownItem(
              'Weekly Tasks', weeklyCompleted, weeklyCount, AppTheme.success),
          const SizedBox(height: 10),
          _buildBreakdownItem('Monthly Tasks', monthlyCompleted, monthlyCount,
              AppTheme.warning),
        ],
      ),
    );
